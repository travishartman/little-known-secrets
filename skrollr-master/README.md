...
#### Let's have the animation end when the top of the element reaches the top of the screen (element leaves the screen)
...
* Skrollr keyframes can either be [absolute](#absolute-mode-or-document-mode) or [relative](#relative-mode-or-screen-mode).
...
### relative mode (or screen mode)

Instead of defining key frames relative to the **document** (i.e. absolute), we are able to define them depending on the position of any element in relation to the **screen**.

The syntax is `data-[offset]-(screen-anchor)-[element-anchor]`, where `offset` can again be any integer and defaults to 0. Both `screen-anchor` (mandatory) and `element-anchor` (optional) can be one of `top`, `center` or `bottom`. If `element-anchor` is ommitted, the value of `screen-anchor` will be taken (just like with background-position). Here are some examples of key frames and their meaning.

* `data-top` = `data-0-top` = `data-top-top` = `data-0-top-top`: When the element's top is aligned with the top of the screen.
* `data-100-top` = `data-100-top-top`: When the element's top is 100px above the top of the screen.
* `data--100-top` = `data--100-top-top`: When the element's top is 100px below the top of the screen.
* `data-top-bottom `= `data-0-top-bottom`: When the bottom of the element is at the top of the screen (it's just not visible).
* `data-center-center` = `data-0-center-center`: When the element is at the center of the screen.
* `data-bottom-center` = `data-0-bottom-center`: When the element's center is at the bottom of the screen, thuss the upper half of the element is visible.
...
I was lying to you. The syntax for absolute mode is not `data-[offset]-[anchor]` and for relative mode it's not `data-[offset]-(screen-anchor)-[element-anchor]`. In both cases `offset` can be preceeded by a constant which can be passed to the `ìnit` method. The name of the constant needs to be preceeded with an underscore.
...
By default skrollr uses the largest key frame and makes document height + screen height this high, thus the max possible scroll top offset. If your animation runs too fast or too slow, just adjust the scale value.
...
`true`: Make sure the document is high enough that all key frames fit inside. Example: You use `data-1000`, but the content only makes the document 500px high. skrollr will ensure that you can scroll down the whole 1000px. Or if you use relative mode, e.g. `data-top-bottom`, skrollr will make sure the bottom of the element can actually reach the top of the screen.
...
### relativeToAbsolute(element, screenAnchor, elementAnchor)
...
`screenAnchor` and `elementAnchor` must be one of `top`, `center` or `bottom`
...
//offset contains the scroll position at which #foo's bottom is at the top of the screen.
//If you now use setScrollTop(offset) or animateTo(offset) #foo's bottom will be perfectly aligned with the top of the screen. Yay.
```
...
Returns the number of pixels that can be scrolled down in total. If `forceHeight` is true, that's usually the largest keyframe. Otherwise it's the height of the document minus the screen height.
